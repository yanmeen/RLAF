...
class DDPG(object):
    ...
    def _build_a(self, s, scope, trainable):
        with tf.variable_scope(scope):
            net = tf.layers.dense(
                s, 20, activation=tf.nn.relu, name='l1', trainable=trainable)
            net1 = tf.layers.dense(
                net, 20, activation=tf.nn.relu, name='l2', trainable=trainable)
            a = tf.layers.dense(
                net1, self.a_dim, activation=tf.nn.softmax, name='a', trainable=trainable)
            return a
